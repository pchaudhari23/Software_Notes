...
- Who are you? - Verifying the user credentials. Is the username and password combination correct. Should the user be allowed to login.
- OAuth2, SSO, Okta
- Cognito
- Microsoft azure AD
...
- What you can do? - Determine which parts of the app the user can access based on his/her role (user/admin) i.e to authorise the user/ is the user authorised to access this part of product?
...
- **Cookies** are **small pieces of data stored in the user’s web browser** by a website.
- Stored on the **client side**, so they **should not store sensitive data directly** because they can be **viewed or manipulated**.
- Have an **expiry period** (minutes, hours, days, or months).
- **A session ID is often stored in a cookie** and is **sent to the server with every request**.

Sessions:

- **Sessions store user data on the server side**, which makes them **better for sensitive data**.
- The user is **identified by a session ID**.
- The **session ID is usually stored in a cookie** on the client.
- Session data exists **until it expires or the user logs out**.
- **Sessions & cookies are part of _authentication_, not authorization** (but they help both).

How it works?

- User login: Server verifies credentials and creates a session.
- Unique Session ID is sent to client inside a set-cookie header and actual user data is stored on server.
- On every request, the session ID is checked with session stored on server. Then server performs authorization checks
- On logout session data is cleared and cookie becomes invalid or expires.

Methods:

1. JWT (Authentication) + RBAC (Authorization) OR
2. OpenID Connect (OIDC) (Authentication) + OAuth 2.0 (Authorization)
   - OAuth 2.0 provides identity token (identity) and access token (authorization)
   - User authenticates via an **Identity Provider (IdP).** Eg: Microsoft Entra ID (Azure AD), Google, Okta, Auth0, Keycloak
   - OIDC is an authentication layer on top of OAuth 2.0. OIDC is a authentication protocol/ standard.
